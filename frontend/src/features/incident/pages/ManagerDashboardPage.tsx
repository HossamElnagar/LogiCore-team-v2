...
import { useNavigate } from "react-router-dom";
import { useSelector } from "react-redux";
import { AlertTriangle, BellRing, CheckCircle2, Clock3, ShieldAlert, TrendingUp, Users, ChevronLeft, ChevronRight } from "lucide-react";
import api from "../../../api/axios";
import { useSocket } from "../../../hooks/useSocket";
import type { RootState } from "../../../app/store";
...
export default function ManagerDashboardPage() {
  ...
  // Fetch Dashboard data with server-side pagination for all sections
  const loadMetrics = useCallback(async (ePage: number, dPage: number, wPage: number) => {
    try {
      setLoading(true);
      const response = await api.get(
        `/analytics/dashboard?page=${ePage}&limit=5&dailyPage=${dPage}&dailyLimit=4&weeklyPage=${wPage}&weeklyLimit=4`
      );
      if (response.data?.success) {
        setMetrics(response.data.data);
      }
  ...
  useEffect(() => {
    void loadMetrics(empPage, dailyPage, weeklyPage);
  }, [empPage, dailyPage, weeklyPage, loadMetrics]);
  ...
  useEffect(() => {
    ...
    socket.on("manager:notification", handleNotification);
    return () => socket.off("manager:notification", handleNotification);
  }, [socket, empPage, dailyPage, weeklyPage, loadMetrics]);
  ...
  const totalEmpPages = metrics?.pagination?.totalPages || 1;
  const totalDailyPages = metrics?.reports?.dailyReport?.totalPages || 1;
  const totalWeeklyPages = metrics?.reports?.weeklyReport?.totalPages || 1;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* CS Employees Section with Server-Side Pagination */}
      <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-extrabold text-slate-900">CS Employees</h2>
          <div className="text-sm text-slate-500">Click an employee to open their detailed summary.</div>
        ...
        <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50"
            >
      ...
      {/* Daily & Weekly Reports with Server-Side Pagination */}
      <div className="grid gap-6 xl:grid-cols-2">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <div className="mt-4 space-y-2">
              {metrics?.reports?.dailyReport?.incidents?.length ? metrics.reports.dailyReport.incidents.map((incident) => (
                <button
                  key={incident._id}
                  type="button"
                  className="flex w-full items-center justify-between rounded-2xl border border-slate-200 px-4 py-3 text-left hover:bg-slate-50 transition"
                  onClick={() => incident._id && navigate(`/incidents/${incident._id}`)}
                >
                  <span className="text-sm font-semibold text-slate-700">{incident.title}</span>
                  <span className="text-xs font-bold uppercase text-slate-500">{incident.status}</span>
                </button>
              )) : <p className="py-2 text-sm text-slate-500">No daily incidents to show.</p>}
            </div>
          </div>
          <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
            <span className="text-xs text-slate-500">Page {dailyPage} of {totalDailyPages}</span>
            <div className="flex items-center gap-2">
              <button
                type="button"
                disabled={dailyPage <= 1}
                onClick={() => setDailyPage((prev) => Math.max(prev - 1, 1))}
                className="flex items-center gap-1 rounded-xl border border-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50"
              >
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <div className="mt-4 space-y-2">
              {metrics?.reports?.weeklyReport?.incidents?.length ? metrics.reports.weeklyReport.incidents.map((incident) => (
                <button
                  key={incident._id}
                  type="button"
                  className="flex w-full items-center justify-between rounded-2xl border border-slate-200 px-4 py-3 text-left hover:bg-slate-50 transition"
              ...
              )) : <p className="py-2 text-sm text-slate-500">No weekly incidents to show.</p>}
            </div>
          </div>
          <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
            <span className="text-xs text-slate-500">Page {weeklyPage} of {totalWeeklyPages}</span>
            <div className="flex items-center gap-2">
              <button
                type="button"
                disabled={weeklyPage <= 1}
                onClick={() => setWeeklyPage((prev) => Math.max(prev - 1, 1))}
                className="flex items-center gap-1 rounded-xl border border-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50"
              >
                <ChevronLeft className="h-3.5 w-3.5" /> Prev
              </button>
              <button
                type="button"
                disabled={weeklyPage >= totalWeeklyPages}
                onClick={() => setWeeklyPage((prev) => Math.min(prev + 1, totalWeeklyPages))}
                className="flex items-center gap-1 rounded-xl border border-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50"
              >
                Next <ChevronRight className="h-3.5 w-3.5" />
              </button>
